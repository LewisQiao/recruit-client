<template>
  <div class="dial">
    <Content :style="{margin: '20px', background: 'f1f1f1'}">
      <!-- <div class="search">
        <label>咨询类型</label>
        <el-select v-model="problemType" placeholder="请选择咨询类型" style="width: 10%;margin-right:30px">
          <el-option label="全部" value=" " key=" " aria-selected="true"></el-option>
          <el-option
            v-for="item in problemTypeEnum"
            :key="item.key"
            :label="item.value"
            :value="item.key"
          ></el-option>
        ...
        <label>问题描述</label>
        <Input v-model="description" style="width: 10%;margin-right:30px"/>
        <label>姓名</label>
            ...
            <div class="add">
              <el-dialog title="修改" :visible.sync="updatedialogFormVisible">
                <el-form :model="updateform" :rules="rules" ref="updateform">
                  ...
                  <el-form-item label="咨询类型" :label-width="formLabelWidth" prop="problemType">
                    <el-select v-model="updateform.problemType" placeholder="请选择咨询类型">
                      <el-option
                        v-for="item in problemTypeEnum"
                        :key="item.key"
                        :label="item.value"
                  ...
                  <el-form-item label="咨询问题级别" :label-width="formLabelWidth" prop="problemLevel">
                    <el-input
                      v-model.trim="updateform.problemLevel"
                      autocomplete="off"
                      type="number"
                    ></el-input>
                  </el-form-item>
                  <el-form-item label="问题状态" :label-width="formLabelWidth" prop="problemStatus">
                    <el-select v-model="updateform.problemStatus" placeholder="请选择问题状态">
                      <el-option label="未解决" value="1" key="1"></el-option>
                      <el-option label="已解决" value="2" key="2"></el-option>
                    </el-select>
                  </el-form-item>
                  ...
                  <el-form-item label="未解决说明" prop="unresRemark" :label-width="formLabelWidth">
                    <el-input
                      v-model="updateform.unresRemark"
                      autocomplete="off"
                      type="textarea"
                      placeholder="当问题状态选择“未解决”时需要填写"
                    ></el-input>
                  </el-form-item>
                  <!-- 隐藏id值 -->
          ...
          <div class="add">
            <el-dialog title="增加" :visible.sync="dialogFormVisible">
              <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="咨询类型" :label-width="formLabelWidth" prop="problemType">
                  <el-select v-model="form.problemType" placeholder="请选择咨询类型">
                    <el-option
                      v-for="item in problemTypeEnum"
                      :key="item.key"
                      :label="item.value"
                ...
                <el-form-item label="咨询问题级别" :label-width="formLabelWidth" prop="problemLevel">
                  <el-input v-model.trim="form.problemLevel" autocomplete="off" type="number"></el-input>
                </el-form-item>
                <el-form-item label="问题状态" :label-width="formLabelWidth" prop="problemStatus">
        ...
        <div class="row" id="pdfDom" style="background-color:#fff;">
          <div class="dataList">
            ...
            >
              ...
              <el-table-column prop="id" label="问题ID" width="150"></el-table-column>
              <el-table-column prop="visitor.visitorName" label="姓名" width="150"></el-table-column>
              <el-table-column prop="visitor.phone" label="电话" width="150"></el-table-column>
              <el-table-column prop="problemTypeName" label="咨询类型"></el-table-column>
              <el-table-column prop="startTimeName" label="问题生成时间" width="180"></el-table-column>
              <el-table-column prop="endTimeName" label="问题结束时间" width="180"></el-table-column>
              <el-table-column label="问题描述">
                <template slot-scope="scope">{{ scope.row.description}}</template>
              </el-table-column>
              <el-table-column prop="problemStatusName" label="问题状态" width="180"></el-table-column>
            </el-table>
            <div class="pageNum">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        problemStatus: [
          { required: true, message: "请选择问题状态", trigger: "blur" }
        ],
        description: [
  ...
  methods: {
    ...
    exportDataFile(type) {
      require.ensure([], () => {
        ...
        exportFile(this.tableData3, "problem", type);
      });
      this.xiazai();
    },
    collapsedSider() {
    ...
    updatedConfirm() {
      ...
      if (selectedLength == 0) {
        ...
        this.updateform.description = this.tableChecked[0].description;
        this.updateform.idCard =
          this.tableChecked[0].visitor == null
            ? ""
            : this.tableChecked[0].visitor.idCard;
        this.updateform.problemType = this.tableChecked[0].problemType;
        this.updateform.problemLevel = this.tableChecked[0].problemLevel;
        this.updateform.problemStatus = this.tableChecked[0].problemStatus;
        this.updateform.unresRemark = this.tableChecked[0].unresRemark;
        // 显示弹出层
    ...
    updateNum(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios
            .post(
              "/apil/vas/problem/update",
              this.$qs.stringify({
                id: this.updateform.id,
                description: this.updateform.description,
                "visitor.idCard": this.updateform.idCard,
                problemType: this.updateform.problemType,
                problemLevel: this.updateform.problemLevel,
                problemStatus: this.updateform.problemStatus,
                unresRemark: this.updateform.unresRemark
              })
            )
            .then(function(res) {
              that.$message(res.data.msg, "info");
    ...
    getproblemTypeEnum() {
      var that = this;
      this.$axios
        .get("/apil/vas/problem/problemType")
        .then(function(res) {
          if (res.data.code == 1) {
            that.problemTypeEnum = res.data.t;
          } else {
            that.$message(res.data.msg, "error");
          }
        })
