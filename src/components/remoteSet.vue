          ...
          <div class="add">
            <el-dialog title="修改" :visible.sync="updatedialogFormVisible">
              <el-form :model="updateform" :rules="rules" ref="updateform">
                ...
                <!-- 隐藏id值 -->
                <el-input v-model="updateform.w_id" v-if="false" />
                <el-form-item label="地址名称" :label-width="formLabelWidth" prop="w_name">
                  <el-input v-model="updateform.w_name" autocomplete="off"></el-input>
                </el-form-item>
                <!-- <el-form-item label="时间" :label-width="formLabelWidth" prop="w_time">
                  <el-input v-model="updateform.w_time" autocomplete="off"></el-input>
                </el-form-item> -->
                <el-form-item label="状态" :label-width="formLabelWidth" prop="w_state">
                  <!-- <el-input v-model="updateform.accessPassword" autocomplete="off"></el-input> -->
                  <el-select v-model="updateform.w_state" placeholder="请选择状态">
        ...
        <div class="add">
          <el-dialog title="增加" :visible.sync="dialogFormVisible">
            <el-form :model="form" :rules="rules" ref="form">
              ...
              <el-form-item label="地址名称" :label-width="formLabelWidth" prop="deviceName">
          ...
          <div class="dataList">
            <el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%"
              ...
              <el-table-column label="ID" width="300">
                <template slot-scope="scope">{{ scope.row.w_id}}</template>
              </el-table-column>
              <el-table-column prop="w_name" label="地址名称"></el-table-column>
              <el-table-column prop="w_time" label="时间" show-overflow-tooltip></el-table-column>
              <el-table-column prop="w_state" label="状态"></el-table-column>
            </el-table>
          </div>
          <div class="pageNum">
  ...
  export default {
    ...
    methods: {
      ...
      checkData(pageNumber) {
        ...
        this.$axios
          .post(
            "/apil/workPlace/selectWorkPlaceList"
          )
          .then(function (res) {
            if (res.data.code == 1) {
            ...
            that.pageCount = res.data.obj.records.w_time;
            var w_state;
      ...
      deletData() {
        ...
        for (let i = 0; i < length; i++) {
          console.log(this.tableChecked[i].w_id);
          ids += this.tableChecked[i].w_id + ",";
        }
        ...
        this.$axios
          .post(
            "/apil/workPlace/deleteWorkPlaceById",
            this.$qs.stringify({
              wid: ids
            })
          )
      ...
      updatedConfirm() {
        ...
        if (selectedLength == 0) {
          ...
          // 获取选中的数据
          var w_id, w_name, w_state;
          console.log(this.tableChecked.length)
          for (let i = 0; i < selectedLength; i++) {
            console.log("for" + selectedLength)
            w_id = this.tableChecked[i].w_id;
            w_name = this.tableChecked[i].w_name;
            w_state = this.tableChecked[i].w_state;
          }
          // 赋值
          this.updateform.w_id = w_id;
          this.updateform.w_name = w_name;
          this.updateform.w_state = w_state;
          // 显示弹出层
      ...
      updateNum() {
        var that = this;
        var w_id = this.updateform.w_id;
        var deviceName = this.updateform.w_name;
        // var ipAddress = this.updateform.ipAddress;
        var accessPassword = this.updateform.w_state;
